...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 80, 'green')
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    center_sequence = generate_points_on_circle(circle, number_of_points)

    for k in range(number_of_points):
        point = center_sequence[k]

        circle = rg.Circle(center_sequence[k], 10)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        end_point_sequence = generate_points_on_circle(circle, number_of_slices)

        line = rg.Line(circle.center, end_point_sequence[k])
        line.color = color
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    initial_line = rg.Line(points[0], points[len(points)-1])

    initial_line.thickness = thickness
    initial_line.color = color

    initial_line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 51, 25, 'black', 2)
    window.close_on_mouse_click()
